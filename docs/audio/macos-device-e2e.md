...
2. Open the patchbay and enumerate inputs. Confirm the intended physical input or BlackHole endpoint is present and capture its stable UID.
3. Route that exact stable UID to a dynamic mixer channel. Confirm the route reaches `active` and visible peak/RMS meters move with a known signal.
4. Change the fader and confirm the master level follows the configured fader law. Toggle MUTE and SOLO and confirm routing semantics without stale audio.
5. Arm local recording and run a continuous live signal for at least **10 seconds**. During the run capture callback count, average/maximum callback duration, xrun count, and recorder/fingerprint queue overflow telemetry.
6. Stop/finalize the recording. Inspect the resulting WAV independently: it must open, have the negotiated sample rate/channel representation expected by the writer, contain non-silent program audio where expected, and have a recording duration of at least 10 seconds.
7. While a route is active, disconnect the physical interface or remove/disable the virtual endpoint. Confirm `deviceLost`/removed state is actionable, the process does not crash, and stale PCM is not silently routed.
8. Reconnect/re-enable the endpoint, refresh discovery, reselect the endpoint by stable UID, and confirm the route can reconnect and return to live metering. Record the disconnect/reconnect result.
...
- Callback count, average callback duration, maximum callback duration, and xrun count.
- Recorder rejected blocks and fingerprint rejected blocks. A clean acceptance run expects zero queue overflow/rejected blocks; any non-zero value must be treated as a failed clean run and investigated.
