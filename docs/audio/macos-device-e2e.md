...
6. Arm local recording and run a continuous live signal for at least **10 seconds**. During the run inspect the **E2E TELEMETRY** surface, then select **COPY EVIDENCE** and paste the generated `## Same-process E2E telemetry` Markdown block into `docs/audio/issue3-device-acceptance-record.md`. The copied block contains callback count, average/maximum callback duration, xrun count, negotiated sample rate/buffer/channels, recorder/fingerprint queue depths, rejected-block counters, and the clean/rejected handoff result. It deliberately contains no endpoint UID, filesystem path, credentials, account data, or hardware serial numbers. These values are sampled on the non-real-time Dart host side from state published by the native engine and PCM pump; the telemetry path must never execute on the Core Audio callback.
...
- The pasted **COPY EVIDENCE** Markdown block from the same Flutter app process, containing callback count, average callback duration, maximum callback duration, xrun count, recorder/fingerprint queue depths, rejected-block counters, and queue-overflow result.
