...
void main() {
  ...
  test('persists and reloads a session tracklist', () async {
    ...
    expect((await store.load()).single.title, 'Title');
    expect(await File('${directory.path}/session.json.tmp').exists(), isFalse);
    await store.dispose();
  });

  test('recovers a valid backup when the primary is missing', () async {
    final file = File('${directory.path}/session.json');
    await File('${file.path}.bak').writeAsString(const SessionTracklistCodec().encode(entries));
    final store = SessionTracklistStore(file: file);
    expect((await store.load()).single.title, 'Title');
    await store.dispose();
  });

  test('recovers a valid backup when the primary is corrupt', () async {
    final file = File('${directory.path}/session.json');
