...
class SessionTracklist {
  SessionTracklist({
    ...
    Iterable<TracklistEntry> initialEntries = const <TracklistEntry>[],
  }) : _entries = List<TracklistEntry>.of(initialEntries);

  final Duration duplicateWindow;
  final List<TracklistEntry> _entries;

  List<TracklistEntry> get entries => List<TracklistEntry>.unmodifiable(_entries);
  ...
  bool addAutomatic(TracklistEntry candidate) {
    if (candidate.provenance != TrackProvenance.automatic) {
      throw ArgumentError.value(
        ...
        'candidate.provenance',
        'Automatic entries only',
      );
    }
    if (shouldSuppressAutomaticMatch(
      candidate: candidate,
      existing: _entries,
      debounceWindow: duplicateWindow,
    )) {
      return false;
    }
    final manualIndex = _entries.indexWhere(
      (entry) =>
  ...
  TracklistEntry correct({
    ...
    final corrected = _entries[index].applyManualCorrection(
      artist: artist,
      title: title,
