...
String redactDiagnostic(String value) {
  ...
  final withoutBearerToken = value.replaceAll(RegExp('bearer${slash}s+${slash}S+', caseSensitive: false), 'Bearer [REDACTED]');
  return withoutBearerToken.replaceAllMapped(RegExp('(authorization|token|password)${slash}s*[:=]${slash}s*[^${slash}s&]+', caseSensitive: false), (match) => '${match.group(1)}=[REDACTED]');
}
