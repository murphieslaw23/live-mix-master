...
/// the Dart host event loop and drains those blocks into negotiated recorder and
/// fingerprint consumers without any callback into Dart from the audio thread.
class NativePcmRuntimePump {
  ...
  void stop({bool drainFinalBlocks = true}) {
    if (_disposed) return;
    if (drainFinalBlocks) drainNow();
    _timer?.cancel();
    _timer = null;
  }
